...
from .baselines.base import BaseExtractor
from .imp import ExtractorFactory
from .utils import calc_rouge_n_score
...
class BaselineData:
    ...
    Contains the raw HTML, ground truth content, meta information, and URL
    ...
    def from_dict(cls, data: dict) -> 'BaselineData':
        ...
        Args:
            data: Dictionary containing benchmark data with keys:
                  ...
                  - 'meta': Dictionary containing 'level', 'language', ... etc.
...
def eval_batch_cases(
    ...
    input_list = [
        (baseline_data.html, baseline_data.url) for baseline_data in baseline_data_list
    ]
    ...
    for baseline_data, result in zip(baseline_data_list, extract_result_list):
        ...
        result_list.append({'track_id': track_id, **rouge_score, **meta})
    return result_list
...
def reduce_results(flat_eval_df: pd.DataFrame, task_dir: str) -> None:
    ...
    separately for each meta information, then saves to JSON file.
    ...
    # ignore track_id,prec,rec,f1
    meta_keys = [
        key
        for key in flat_eval_df.columns
        if key not in ['track_id', 'prec', 'rec', 'f1']
    ]
    for meta_key in meta_keys:
        for meta_value in flat_eval_df[meta_key].unique():
            meta_value_mean_dict = {}
            for metric in flat_eval_df.columns:
                try:
                    meta_value_mean_dict[metric] = flat_eval_df[
                        flat_eval_df[meta_key] == meta_value
                    ][metric].mean()
                except TypeError:
                    # Skip non-numeric columns
                    pass
            mean_dict[f'{meta_key}.{meta_value}'] = meta_value_mean_dict
...
class SingleProcessMaper:
    ...
    def __init__(
        ...
        Args:
            ...
            extractor_name: Name of the extractor to use (e.g., 'mineru_html-html-md')
    ...
    def run(self) -> list[dict]:
        ...
        for baseline_data in tqdm(self.benchmark_dataset.values()):
...
class RayBatchProcessMaper:
    ...
    def run(self) -> list[dict]:
        ...
        tasks = [
            ...
            ).remote(batch, cases_dir_str, self.extractor_name, self.extractor_config)
